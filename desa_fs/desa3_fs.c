...
void
desa2_freeswitch(int16_t *input, double *mean1, double *mean2, double *var1, double *var2)
{
    int i;
    circ_buffer_t   b;
	sma_buffer_t    sma_b;
	...
	*var1 = sqa_b.sma - (sma_b.sma * sma_b.sma);
	printf("<<< AVMD v[%f] f[%f][%f]Hz sma[%f][%f]Hz sqa[%f] >>>\n",
            *var1, freq[i], TO_HZ(8000, freq[i]), sma_b.sma, TO_HZ(8000, sma_b.sma), sqa_b.sma);
    }
    /* set mean */
    *mean1 = sma_b.sma;
...
main(int argc, char *argv[])
{
    ...
    while( numWords == BLOCK )
    {
        ...
        desa2_freeswitch(intData, &frequency, &freq2, &variance, &var2);
        printf("\nDesa2_fs: Mean freq = %f, var = %f, std dev = %f\n, freq2 = %f, var2 = %f",
